...
fn render_top_bar(
    ...
    #[cfg(all(feature = "native-file-dialog", not(target_arch = "wasm32")))]
    {
        if ui.button("Load PRG...").clicked() {
            ...
            {
                let result =
                    emulator.run_program(ProgramSource::File(path.clone()), None, None, None, None);
...
fn render_sprite_layer(
    sprite_snapshot: Option<bus::sprite_mmio::SpriteSnapshot>,
    sprite_catalog: &SpriteCatalog,
    sprite_viewport: &SpriteViewport,
    sprite_virtual: &SpriteVirtualResolution,
...
fn render_console_tab(
    ui: &mut egui::Ui,
    console_snapshot: &Option<bus::console_mmio::ConsoleSnapshot>,
) {
            ...
            if snapshot.pads.is_empty() {
                ...
                ui.columns(snapshot.pads.len(), |columns| {
                    for (offset, column) in columns.iter_mut().enumerate() {
                        ...
                        let label = labels.get(pad_index).map(|s| s.as_str()).unwrap_or("None");
...
pub(super) fn ui_system(
    ...
    egui::TopBottomPanel::top("top_panel")
        ...
        .show(ctx, |ui| {
            ui.horizontal(|ui| {
                render_top_bar(ui, &mut emulator, &mut ui_state, &mut controller_state);
            });
        });
    ...
    egui::TopBottomPanel::bottom("console_panel")
        ...
        .show_animated(ctx, ui_state.console_open, |ui| {
            ...
            match ui_state.active_tab {
                ConsoleTab::Console => render_console_tab(ui, &console_snapshot),
                ConsoleTab::Interrupts => render_interrupts_tab(ui, &emulator, &bindings),
                ConsoleTab::Input => render_input_tab(ui, &mut controller_state, &keyboard_tracker),
            }
        });
}
